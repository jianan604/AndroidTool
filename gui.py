...
class AndroidTools:
    ...
    def run_monkey(self):
        ...
        try:
            ...
            if not os.path.exists(work_path):
                os.mkdir(work_path)
            utils.clearLog()
            work_path = os.path.join(work_path, package_name + utils.getCurrentTime())
            # package_name + utils.getCurrentTime()
            if not os.path.exists(work_path):
                os.mkdir(work_path)
            self.workPath = work_path
            t = multiprocessing.Process(target=lambda: self.mk.merge_command(work_path,
    ...
    def analyse(self, log_path):
        task = {}

        if platform.system() == "Windows":
            crash_count = os.popen('type %s|find "CRASH:" /c' % (os.path.join(log_path, 'monkey.log'),)).read().strip()
            anr_count = os.popen('type %s|find "ANR:" /c' % (os.path.join(log_path, 'monkey.log'),)).read().strip()
        else:
            crash_count = os.popen('grep -c "CRASH:" %s' % (os.path.join(log_path, 'monkey.log'),)).read().strip()
            anr_count = os.popen('grep -c "ANR:" %s' % (os.path.join(log_path, 'monkey.log'),)).read().strip()

        task["logPath"] = log_path
        task["crashCount"] = crash_count
        task["anrCount"] = anr_count
        if crash_count == "":
            self.tasksCrashCount = 0
            self.tasksAnrCount = 0
        else:
            self.tasksCrashCount += int(crash_count)
            self.tasksAnrCount += int(anr_count)

        self.tasks["tasks"].append(task)


    def collect(self,work_path):
