...
class Monkey:
    def __init__(self):
        ...
        self.tasks = {}
        self.tasks["pkgName"] = self.cf.get('monkey_conf', 'package_name')
        # self.tasks["duration"] = self.hours
        self.tasks["tasks"] = []
        #self.tasks["deviceInfo"] = device_name
        # add end
    ...
    def merge_command(self,path, *args):
       ...
       # currentPath = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        #package_name = self.cf.get('monkey_conf', 'package_name')
        member = ' '.join(args)
        path1 = path + "/monkey.log"
        #command = 'adb shell monkey {} > {}'.format(member, path)
        command = 'adb shell monkey {} > {}'.format(member,path1)
       # workPath = os.path.join(currentPath, "logs", package_name + utils.getCurrentTime())
        #os.mkdir(workPath)
        self.run(command)
